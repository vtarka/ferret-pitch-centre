%% Produces series of figures showing pitch neurons across 3 response windows
% DEPENDENCIES:
...
for pen = 1:length(PN_units)
    ...
    stims = {'high','low','CT0','allHarm','tone'};
    Flist = unique(F0);
    repeats = unique(Y(:,5));
    allUnits = unique(Y(:,3));
    ...
    for uu = 1:size(PNUnits,1)
        ...
        if length(find(PNUnits==PNUnits(uu,1)))<2 && PNUnits(uu,2)==1

            nexttile
            plot_tuning_by_cond(Y,type,F0,PNUnits(uu,1),stims,zeros(length(stims),1),PN_units{pen,1},PN_units{pen,2},window);
            xlabel('')
            ylabel('')
            xticks([])
            yticks([])
...
sgtitle('Fast onset ONLY sensitive PNs','fontsize',32)
...
for pen = 1:length(PN_units)
    ...
    for uu = 1:size(PNUnits,1)
        ...
        if length(find(PNUnits==PNUnits(uu,1)))<2 && PNUnits(uu,2)==2
...
sgtitle('Slow onset ONLY sensitive PNs','fontsize',32)
...
for pen = 1:length(PN_units)
    ...
    for uu = 1:size(PNUnits,1)
        ...
        if length(find(PNUnits==PNUnits(uu,1)))<2 && PNUnits(uu,2)==3
...
sgtitle('Offset ONLY sensitive PNs','fontsize',32)
...
for pen = 1:length(PN_units)
    ...
    for uu = 1:size(PNUnits,1)
        ...
        if length(find(PNUnits==PNUnits(uu,1)))==2 && ~skip_next
            ...
            subplot(3,2,sp_counter)
            plot_tuning_by_cond(Y,type,F0,PNUnits(uu,1),stims,zeros(length(stims),1),PN_units{pen,1},PN_units{pen,2},windows(1,:));
            xlabel('')
            ylabel('')
            xticks([])
            yticks([])

            subplot(3,2,sp_counter + 1)
            plot_tuning_by_cond(Y,type,F0,PNUnits(uu,1),stims,zeros(length(stims),1),PN_units{pen,1},PN_units{pen,2},windows(2,:));
            xlabel('')
            ylabel('')
            xticks([])
            yticks([])
...
for pen = 1:length(PN_units)
    ...
    for uu = 1:size(PNUnits,1)
        ...
        if length(find(PNUnits==PNUnits(uu,1)))==3 && ~skip_next
...
%             end
            ...
            plot_tuning_by_cond(Y,type,F0,PNUnits(uu,1),stims,zeros(length(stims),1),PN_units{pen,1},PN_units{pen,2},windows(3,:));
            xlabel('')
            ylabel('')
            xticks([])
            yticks([])
