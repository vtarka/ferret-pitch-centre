...
windows = [300:5:450; 305:5:455]'/1000;
...
stims_for_profile = {'CT0','CT5','CT10','CT20','CT40'};
...
for ap = 1:length(Animals)
    ...
    for uu = 1:length(units)
        ...
        rp = get_response_profile(Y,type,F0,units(uu),stims_for_profile,window);

        rp = reshape(rp,[],1);
        all_unit_rps(uCounter,:) = rp;
...
active_units = zeros(size(all_unit_rps,1),1);

% for every unit
for uu = 1:size(all_unit_rps,1)

    % check if there were no spikes whatsoever for this unit
    if ~isempty(find(all_unit_rps(uu,:),1))ylabel('Normalized Firing Rate')
% xlabel('ms after stimulus onset')
        active_units(uu) = 1;
    end
...
active_all_unit_psth = all_unit_rps(active_units==1,:);
...
for uu = 1:size(active_all_unit_psth,1)
...
for i = 1:k
    ...
    if ~isempty(thisCluster)
        ...
        meanCluster = mean(thisCluster);
        meanCluster2d = reshape(meanCluster,5,17);

        subplot(3,2,i); 
        imagesc(meanCluster2d);
        set(gca,'fontsize',22)
    end
