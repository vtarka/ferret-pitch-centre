...
%% Determine whether cells are F0-sensitive in windows determined by their clusters
% Cluster 1: [0 80] 
% Cluster 2: [0 150]
...
for ap = 1:length(Animals)
    ...
    cluster_sensitivity = zeros(length(units),length(stims));
    ...
    for uu = 1:length(units)
        ...
        else % if the unit wasn't active or is one of the bad clusters, don't look at it
            continue
        end
        ...
        if found_sensitive == 0 
            not_sensitive = not_sensitive + 1;
            not_sensitive_c(end+1) = clusters(uu); %[not_sensitive_c; cluster(uu)];
        end

        if strcmp(plot_yn,'y')
            if ~isempty(find(cluster_sensitivity(uu,:), 1))

                stim_to_plot = {};
                for ss = 1:length(stims)
                
                    if cluster_sensitivity(uu,ss)==1
                ...
                title(sprintf('Cluster %d',clusters(uu)))
    ...
    save(['/media/veronica/Kat Data/Veronica/pitch_ephys/DansMATLABData/' Animals{ap} '/tmp01/Spikes_' Animals{ap} '_' Pens{ap} '_Good_Pitch.mat'],...
       'Y','type','F0','cluster_sensitivity','clusters')
